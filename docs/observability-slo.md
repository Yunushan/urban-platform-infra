...
This project should be observable before it is scaled. The default stack includes Elasticsearch/Kibana/Logstash scaffolding and optional kube-prometheus-stack installation through Helmfile. SLO and alerting resources are disabled by default in the application chart so the chart can render without Prometheus Operator CRDs, then enabled after `kube-prometheus-stack` is installed.

## Article 8 Baseline

The production baseline is:

1. Install kube-prometheus-stack through `deploy/helmfile.yaml`.
