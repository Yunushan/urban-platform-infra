...
`/dashboard` away from the baked frontend. Static frontend Services are
normalized to a plain service port `80` behind Traefik, with the Service target
port mapped to the generated unprivileged nginx listener. This keeps the
Ingress backend stable even when the old Compose edge service exposed `443`,
`8080`, or another external port. Hostless IP-based Traefik routes are emitted
with a high explicit router priority so older/default or path-specific hostless
routes do not take `/login` or `/dashboard` traffic away from the imported
frontend.
...
- `MIGRATION_IMAGE_MODE=preload` builds images, saves each image as a tar
  ...
  are not sent again.
  `MIGRATION_CLEANUP_NODE_IMAGE_SCOPE=desired` is the HA-safe production default:
  every node preserves every currently desired imported image alias so workloads
  can be rescheduled without a registry. Lab profile defaults cleanup to
  `scheduled`, which keeps only imported image aliases used by pods currently
  scheduled on each node. Scheduled scope can reclaim much more RKE2/containerd
  disk, but a later pod move to another node may require rerunning
  `MIGRATION_STAGE=images` unless a private registry is available.
  ...
  retained. Full-scope manifest applies also prune stale imported Deployment and
  Service resources that are no longer generated by the current Compose project,
  so renamed or removed services do not keep old pods, ReplicaSets, and image
  references alive.
