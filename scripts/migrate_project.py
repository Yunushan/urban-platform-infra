...
def full_import_cleanup_is_safe(
    args: argparse.Namespace,
    service_pairs: list[tuple[import_project.ServiceRecord, dict[str, Any]]],
    resource_label: str,
) -> bool:
    if service_filter_terms(args):
        print(f"Skipping stale imported {resource_label} cleanup because MIGRATION_SERVICE_FILTER limits the selected services.")
        return False
    plan = import_batch_plan(args, service_pairs)
    if plan["selectedBatch"] is not None:
        print(f"Skipping stale imported {resource_label} cleanup because MIGRATION_IMPORT_BATCH does not select the full import set.")
        return False
    return True


def cleanup_stale_import_workload_resources(
    args: argparse.Namespace,
    ...
    if not args.execute:
        return
    if not full_import_cleanup_is_safe(args, service_pairs, "workload resource"):
        return
    current_names_by_kind: dict[str, set[str]] = {"Deployment": set(), "Service": set()}
    for manifest in workload_manifests:
        kind = str(manifest.get("kind") or "")
        if kind in current_names_by_kind and metadata_name(manifest):
            current_names_by_kind[kind].add(metadata_name(manifest))
    label_selector = "app.kubernetes.io/managed-by=urban-platform-import,app.kubernetes.io/part-of=urban-platform-import"
    resource_kinds = [("deploy", "Deployment"), ("service", "Service")]
    for resource, kind in resource_kinds:
        current_names = current_names_by_kind[kind]
        if not current_names:
            continue
        try:
            items = kubectl_json(args, ["-n", args.namespace, "get", resource, "-l", label_selector]).get("items", [])
        except RuntimeError as exc:
            print(f"Could not inspect stale imported {kind} resources for cleanup: {compact_runtime_message(exc)}")
            continue
        stale_names = [
            str((item.get("metadata", {}) or {}).get("name") or "")
            for item in items
            if (item.get("metadata", {}) or {}).get("name")
            and str((item.get("metadata", {}) or {}).get("name") or "") not in current_names
        ]
        if not stale_names:
            continue
        print(f"Removing stale imported {kind} resource(s) that are no longer generated by the current full manifest set.")
        for name in sorted(set(stale_names)):
            run_command(kubectl_command(args, ["-n", args.namespace, "delete", resource, name, "--ignore-not-found=true"]))
...
def stage_manifests(args: argparse.Namespace, service_pairs: list[tuple[import_project.ServiceRecord, dict[str, Any]]], values: dict[str, Any]) -> None:
    ...
    if args.execute and workload_manifests:
        ...
        cleanup_stale_import_workload_resources(args, workload_manifests, service_pairs)
